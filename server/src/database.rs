...
pub fn add_event_json(data: String, conn: &mut PgConnection) -> Option<i64> {
	/*
	let event_struct: Result<EventJson, serde_json::Error> = serde_json::from_str(&data.as_str());
	if event_struct.is_err() {
		None
	}

	diesel::insert_into(event).values((
		event::name.eq(event_struct.unwrap().name),
		event::description_text.eq(event_struct.unwrap().description),
		event::start.eq(event_struct.unwrap().start),
	));
